    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_rightbra:
                ...
                break;
            case R.id.btn_equ:
                //如果字符串的最后一个字符是加/减/乘/除，则代表表达式不完整，用Toast给出错误信息
                if (str.charAt(str.length() - 1) == '+' || str.charAt(str.length() - 1) == '-' || str.charAt(str.length() - 1) == '×' || str.charAt(str.length() - 1) == '÷') {
                    Toast.makeText(MainActivity.this, "Please complete the formula!", Toast.LENGTH_SHORT).show();
                    text.setText(str);
                }
                //如果表达式完整，则先调用isMatched方法验证括号是否匹配
                else {
                    String ero = isMatched(str);
                    //如果括号匹配，则调用getResult方法获取计算结果
                    //如果括号不匹配，则用Toast显示错误信息
                    if (ero.equals("no error")) {
                        String re = getResult();
                        //如果返回的计算结果是“无穷”，则用Toast和在展示区显示错误信息
                        //否则，在展示区显示计算结果
                ...
                break;
            case R.id.btn_squ2:
                //如果以负号开头或表达式含有加减乘除，则用Toast显示报错信息
                //如果正常，则先转为浮点数，开方，再转回字符串，用正则表达式处理格式
                //下面的百分号、sin、cos等计算同理，（负号开头的数可以进行这些计算）
                if (str.charAt(0) == '-') {
            ...
            case R.id.btn_percent:
                if (str.contains("+") || str.contains("×") || str.contains("÷") || str.contains("(") || str.contains(")") || str.lastIndexOf('-') > 0) {
                    Toast.makeText(MainActivity.this, "Symbols cannot be percented!", Toast.LENGTH_SHORT).show();
                    //text.setText("0");
                } else {
            ...
            case R.id.btn_sin:
                if (str.contains("+") || str.contains("×") || str.contains("÷") || str.contains("(") || str.contains(")") || str.lastIndexOf('-') > 0) {
                    Toast.makeText(MainActivity.this, "Symbols cannot be SINED!", Toast.LENGTH_SHORT).show();
                    //text.setText("0");
                } else {
            ...
            case R.id.btn_cos:
                if (str.contains("+") || str.contains("×") || str.contains("÷") || str.contains("(") || str.contains(")") || str.lastIndexOf('-') > 0) {
                    Toast.makeText(MainActivity.this, "Symbols cannot be COSINED!", Toast.LENGTH_SHORT).show();
                    //text.setText("0");
                } else {
                ...
                break;
            case R.id.btn_PandM:
                //正负号转换：0显示0，否则如果开头不是负号，则添加负号
                //如果开头是负号，则用substring去掉负号
